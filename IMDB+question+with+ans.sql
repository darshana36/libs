...
FROM   (SELECT m.id,
        ...
        FROM   movie m
               JOIN ratings r
                 ON m.id = r.movie_id
        WHERE  Date(date_published) BETWEEN '2018-04-01' AND '2019-04-01')t2
...
SELECT ac.NAME,
       ...
       Rank()
         OVER(
           ORDER BY rc.actor_avg_rating DESC) AS actor_rank
FROM   actor_count ac
       INNER JOIN rating_count rc
               ON ac.NAME = rc.NAME
WHERE  movie_count >=5
...
WITH actress_count
     ...
     AS (SELECT n.NAME,
         ...
         FROM   movie m
                ...
                JOIN names n
                  ON rm.name_id = n.id
         WHERE  m.languages = 'Hindi'
         GROUP  BY n.NAME)
SELECT ac.NAME,
       rc.total_votes,
       ac.movie_count,
       rc.actress_avg_rating,
       Rank()
         OVER(
           ORDER BY rc.actress_avg_rating DESC) AS actress_rank
FROM   actress_count ac
       INNER JOIN rating_count rc
               ON ac.NAME = rc.NAME
WHERE  movie_count >=3
...
-- Top five actresses in Hindi movies released in India based on their average ratings are Taapsee Pannu, Shraddha Kapoor and Kriti Sanon
...
AS
  (
             SELECT     genre,
                        year,
                        title                                                                                                                                     AS movie_name,
                        cast(REPLACE(REPLACE(ifnull(worlwide_gross_income,0),'INR',''),'$','') AS DECIMAL(10))                                                    AS world_wide,
...
(
           ...
           ON         m.id = d.movie_id
           INNER JOIN ratings AS r
           ON         r.movie_id = m.id ), top_director_info AS
(
       SELECT *,
